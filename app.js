...
if (forgotPassLink) {
  forgotPassLink.addEventListener("click", async (e) => {
    ...
    if (!email) {
      ...
      errorMsg.innerHTML =
        '<i class="fa-solid fa-circle-exclamation"></i> Please enter your email address first.';
      return;
    }

    errorMsg.style.color = "#666";
    errorMsg.innerHTML =
      '<i class="fa-solid fa-circle-notch fa-spin"></i> Checking account details...';
    ...
    try {
      ...
      if (querySnapshot.empty) {
        ...
        errorMsg.innerHTML =
          '<i class="fa-solid fa-circle-xmark"></i> No account found with this email.';
        return;
      }
      ...
      const inputName = prompt(
        `Security Check: To reset the password for ${email}, please enter your Full Name exactly as registered:`
      );
      ...
      if (dbName !== userProvidedName) {
        errorMsg.style.color = "red";
        errorMsg.innerHTML =
          '<i class="fa-solid fa-shield-halved"></i> <strong>Verification Failed.</strong> The name provided does not match our records.';
        return;
      }

      errorMsg.innerHTML =
        '<i class="fa-solid fa-circle-notch fa-spin"></i> Sending reset link...';
      await sendPasswordResetEmail(auth, email);

      errorMsg.style.color = "green";
      errorMsg.innerHTML =
        '<i class="fa-solid fa-check-circle"></i> Verification passed! Reset link sent.';
    } catch (error) {
...
if (loginForm) {
  loginForm.addEventListener("submit", async (e) => {
    ...
    errorMsg.innerHTML =
      '<i class="fa-solid fa-circle-notch fa-spin"></i> Verifying credentials...';
    errorMsg.style.color = "#666";
